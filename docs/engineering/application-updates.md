...
代理启用时，更新请求经该代理发出；代理未启用（直连模式）时更新流量直连目标。回环目标
（`localhost`、`127.0.0.1`、`::1` 及映射形式）始终直连，因此 QA 回环 updater 端点
（`AI_ROUTER_QA_UPDATER_ENDPOINT`）语义不变。更新路径不咨询 macOS 系统代理/PAC 与环境变量代理，
与全局代理设置的整体行为一致。

代理不可达时更新失败关闭，不会自动回退到直连：手动检查显示有界、可重试的错误，自动检查保持安静并
按既有失败节奏重试，用户把设置切回“直连”即可恢复更新。设置页的“测试连接”只验证代理端点连通性，
不验证 GitHub 可达性，测试成功不代表更新服务可达。代理设置变更在下一个操作（下一次检查或下一次
下载）生效，不重启进程，也不影响已在飞行中的请求。
